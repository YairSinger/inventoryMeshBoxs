...
static void add_ambiguous(imb_session_t *s, const char *uid)
{
    if (find_uid(s->ambiguous, s->ambiguous_count, uid) < 0 &&
        s->ambiguous_count < IMB_REGISTRY_MAX_ITEMS) {
        strncpy(s->ambiguous[s->ambiguous_count].uid, uid, IMB_UID_LEN - 1);
...
void imb_session_apply(imb_session_t *s, const imb_scan_event_t *event)
{
    if (event->dir == IMB_INSERT) {
        ...
        if (existing >= 0) {
            ...
            remove_at(s->present, &s->present_count, (uint16_t)existing);
            add_ambiguous(s, event->uid);
        } else if (s->present_count < IMB_REGISTRY_MAX_ITEMS) {
